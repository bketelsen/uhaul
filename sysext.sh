...
} >"${SYSEXTNAME}/usr/lib/extension-release.d/extension-release.${SYSEXTNAME}.sysext"
rm -f "${SYSEXTNAME}".sysext.raw
if [ "${FORMAT}" = "btrfs" ]; then
  # Note: We didn't chown to root:root, meaning that the file ownership is left as is
  mkfs.btrfs --mixed -m single -d single --shrink --rootdir "${SYSEXTNAME}" "${SYSEXTNAME}".sysext.raw
  ...
  truncate -s 1G "${SYSEXTNAME}".sysext.raw
  # Note: We didn't chown to root:root, meaning that the file ownership is left as is
  mkfs."${FORMAT}" -E root_owner=0:0 -d "${SYSEXTNAME}" "${SYSEXTNAME}".raw
  resize2fs -M "${SYSEXTNAME}".sysext.raw
else
  mksquashfs "${SYSEXTNAME}" "${SYSEXTNAME}".sysext.raw -all-root
fi
echo "Created ${SYSEXTNAME}.sysext.raw"
